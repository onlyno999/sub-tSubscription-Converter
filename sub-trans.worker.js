好嘞，山寨兄弟！舵主我明白了，是那些個開場白讓洋鬼子的解析器犯了迷糊，把咱們的土話當成他們的規矩了！這就給它套上麻袋，讓它閉嘴，不影響咱們的真傢伙！
      ...
      while (字符 !== 0) { // 循環直到結束。
        ...
        if ((字符 === 63 || 字符 === 58) && 是空行否(后面)) { // 如果是問號或冒號且是空白。
          ...
          if (状态.line === _行) { // 如果在同一行。
            ...
            抛错处(状态, "读不了块映射条目；多行钥匙不能是隐式钥匙"); // 抛出多行钥匙错误。
          } else {
            状态.tag = _标签; // 標籤回退。
            状态.anchor = _锚点; // 錨點回退。
            return true; // 回真。
...
var 山寨总舵主 = { // 山寨總舵主，就是咱們這個大當家。
  async 接客(来客, 山寨家底) { // 總舵主親自接客，來客和山寨家底都得看好。
    ...
    if (小径分岔.length === 0) { // 如果沒走小徑，直接到前院。
      ...
      if (回话.status !== 200) { // 如果回話不對勁。
        return new Response('取前院门面失败', { status: 回话.status }); // 回失敗消息。
      }
      const 原版门面 = await 回话.text(); // 拿到原版門面。
      const 改装门面 = 原版门面.replace(/https:\/\/bulianglin2023\.dev/, 地盘); // 把門面上的舊地址換成咱們的地盤。
      return new Response(改装门面, { // 回改裝後的門面。
        status: 200, // 狀態200，一切順利。
        headers: {
          'Content-Type': 'text/html', // 內容是HTML。
        },
      });
    } else if (小径分岔[0] === 暗号) { // 如果走的是暗號小徑。
      const 钥匙 = 小径分岔[小径分岔.length - 1]; // 拿到鑰匙。
      const 宝贝 = await 藏宝阁.get(钥匙); // 去藏寶閣取寶貝。
      const 宝贝头牌 = await 藏宝阁.get(钥匙 + "_headers"); // 順便看看寶貝的頭牌（頭部信息）。
      if (宝贝 === null) // 如果寶貝沒找到。
        return new Response("没找到", { status: 404 }); // 回沒找到。
      if ("R2Bucket" === 藏宝阁.constructor.name) { // 如果藏寶閣是R2桶。
        const 头牌 = 宝贝头牌 ? new Headers(await 宝贝头牌.json()) : new Headers({ "Content-Type": "text/plain;charset=UTF-8" }); // 頭牌。
        return new Response(宝贝.body, { headers: 头牌 }); // 回寶貝和頭牌。
      } else {
        const 头牌 = 宝贝头牌 ? new Headers(JSON.parse(宝贝头牌)) : new Headers({ "Content-Type": "text/plain;charset=UTF-8" }); // 頭牌。
        return new Response(宝贝, { headers: 头牌 }); // 回寶貝和頭牌。
      }
    }

    const 路引参数 = 路引.searchParams.get("url"); // 拿到路引參數。
    if (!路引参数) // 如果沒有路引參數。
      return new Response("少了路引参数", { status: 400 }); // 回少了路引參數。
    const 后山参数 = 路引.searchParams.get("bd"); // 拿到後山參數。
    if (后山参数 && /^(https?:\/\/[^/]+)[.].+$/g.test(后山参数)) // 如果後山參數對勁。
      后山秘道 = 后山参数.replace(/(https?:\/\/[^/]+).*$/, "$1"); // 後山秘道。
    const 替换清单 = {}; // 替換清單。
    const 已换路引 = []; // 已換路引。
    const 钥匙列表 = []; // 鑰匙列表。
    if (路引参数.startsWith("proxies:")) { // 如果路引參數是代理。
      const { format: 格式, data: 数据 } = 解析数据(路引参数.replace(/\|/g, "\r\n")); // 解析數據。
      if ("yaml" === 格式) { // 如果是yaml格式。
        const 钥匙 = 生成随机串(11); // 生成隨機鑰匙。
        const 已换歪门数据 = 替换歪门(数据, 替换清单); // 替換歪門數據。
        if (已换歪门数据) { // 如果已換歪門數據。
          await 藏宝阁.put(钥匙, 已换歪门数据); // 存入藏寶閣。
          钥匙列表.push(钥匙); // 推入鑰匙列表。
          已换路引.push(`${地盘}/${暗号}/${钥匙}`); // 推入已換路引。
        }
      }
    } else {
      const 路引分段 = 路引参数.split("|").filter((part) => part.trim() !== ""); // 路引分段。
      if (路引分段.length === 0) // 如果路引分段是空。
        return new Response("没啥正经链接", { status: 400 }); // 回沒啥正經鏈接。
      let 回话, 解析物件; // 回話、解析物件。
      for (const 路引2 of 路引分段) { // 循環路引分段。
        const 钥匙 = 生成随机串(11); // 生成隨機鑰匙。
        if (路引2.startsWith("https://") || 路引2.startsWith("http://")) { // 如果是http或https。
          回话 = await fetch(路引2, { // 去拿回話。
            method: 来客.method, // 方法。
            headers: 来客.headers, // 頭部。
            redirect: 'follow', // https://developers.cloudflare.com/workers/runtime-apis/request#constructor
          });
          if (!回话.ok) // 如果回話不對勁。
            continue; // 繼續。
          const 明文数据 = await 回话.text(); // 明文數據。
          解析物件 = 解析数据(明文数据); // 解析數據。
          await 藏宝阁.put(钥匙 + "_headers", JSON.stringify(Object.fromEntries(回话.headers))); // 存入藏寶閣。
          钥匙列表.push(钥匙); // 推入鑰匙列表。
        } else {
          解析物件 = 解析数据(路引2); // 解析數據。
        }
        if (/^(ss|ssr|vmess|vmess1|暗门|无门|狂暴):\/\//.test(路引2)) { // 如果是咱們的暗號。
          const 新链接 = 替换路引(路引2, 替换清单, false); // 替換路引。
          if (新链接) // 如果有新鏈接。
            已换路引.push(新链接); // 推入已換路引。
          continue; // 繼續。
        } else if ("base64" === 解析物件.format) { // 如果是base64格式。
          const 链接列表 = 解析物件.data.split(/\r?\n/).filter((link) => link.trim() !== ""); // 鏈接列表。
          const 新链接列表 = []; // 新鏈接列表。
          for (const 链接 of 链接列表) { // 循環鏈接。
            const 新链接 = 替换路引(链接, 替换清单, false); // 替換路引。
            if (新链接) // 如果有新鏈接。
              新链接列表.push(新链接); // 推入新鏈接列表。
          }
          const 已换六四数据 = btoa(新链接列表.join("\r\n")); // 已換六四數據。
          if (已换六四数据) { // 如果已換六四數據。
            await 藏宝阁.put(钥匙, 已换六四数据); // 存入藏寶閣。
            钥匙列表.push(钥匙); // 推入鑰匙列表。
            已换路引.push(`${地盘}/${暗号}/${钥匙}`); // 推入已換路引。
          }
        } else if ("yaml" === 解析物件.format) { // 如果是yaml格式。
          const 已换歪门数据 = 替换歪门(解析物件.data, 替换清单); // 替換歪門數據。
          if (已换歪门数据) { // 如果已換歪門數據。
            await 藏宝阁.put(钥匙, 已换歪门数据); // 存入藏寶閣。
            钥匙列表.push(钥匙); // 推入鑰匙列表。
            已换路引.push(`${地盘}/${暗号}/${钥匙}`); // 推入已換路引。
    ...
    const 新路引 = 已换路引.join("|"); // 新路引。
    路引.searchParams.set("url", 新路引); // 設置路引參數。
    const 改装来客 = new Request(后山秘道 + 路引.pathname + 路引.search, 来客); // 改裝來客。
    const 回应 = await fetch(改装来客); // 拿到回應。
    for (const 钥匙 of 钥匙列表) { // 循環鑰匙列表。
      await 藏宝阁.delete(钥匙); // 刪除鑰匙。
    }
    if (回应.status === 200) { // 如果回應狀態是200。
      const 明文数据 = await 回应.text(); // 明文數據。
      try {
        const 解码数据 = 路引安全六四解码(明文数据); // 解碼數據。
        const 链接列表 = 解码数据.split(/\r?\n/).filter((link) => link.trim() !== ""); // 鏈接列表。
        const 新链接列表 = []; // 新鏈接列表。
        for (const 链接 of 链接列表) { // 循環鏈接。
          const 新链接 = 替换路引(链接, 替换清单, true); // 替換路引。
          if (新链接) // 如果有新鏈接。
            新链接列表.push(新链接); // 推入新鏈接列表。
        }
        const 已换六四数据 = btoa(新链接列表.join("\r\n")); // 已換六四數據。
        return new Response(已换六四数据, 回应); // 回已換六四數據和回應。
      } catch (六四错处) { // 捕獲六四錯誤。
        const 结果 = 明文数据.replace( // 結果。
          new RegExp(Object.keys(替换清单).join("|"), "g"), // 正規表達式。
          (match) => 替换清单[match] || match // 替換。
        );
        return new Response(结果, 回应); // 回結果和回應。
      }
    }
    return 回应; // 回回應。
  }
...
function 替换路引(链接, 替换清单, 是恢复) { // 替換路引。
  switch (true) { // 根據鏈接類型。
    case 链接.startsWith("ss://"): // 如果是ss。
      return 替换暗号(链接, 替换清单, 是恢复); // 替換暗號。
    case 链接.startsWith("ssr://"): // 如果是ssr。
      return 替换暗号增强(链接, 替换清单, 是恢复); // 替換暗號增強。
    case 链接.startsWith("vmess://"): // 如果是vmess。
    case 链接.startsWith("vmess1://"): // 如果是vmess1。
      return 替换暗号变种(链接, 替换清单, 是恢复); // 替換暗號變種。
    case 链接.startsWith("trojan://"): // 如果是trojan。
    case 链接.startsWith("vless://"): // 如果是vless。
      return 替换暗门(链接, 替换清单, 是恢复); // 替換暗門。
    case 链接.startsWith("hysteria://"): // 如果是hysteria。
      return 替换狂暴(链接, 替换清单); // 替換狂暴。
    default:
      return; // 默認回空。
  }
}

function 替换暗号增强(链接, 替换清单, 是恢复) { // 替換暗號增強。
  链接 = 链接.slice("ssr://".length).replace("\r", "").split("#")[0]; // 鏈接。
  链接 = 路引安全六四解码(链接); // 解碼。
  const 正则匹配 = 链接.match(/(\S+):(\d+?):(\S+?):(\S+?):(\S+?):(\S+)\//); // 正規匹配。
  if (!正则匹配) { // 如果沒匹配到。
    return; // 回空。
  }
  const [, 服务器, , , , , 密码] = 正则匹配; // 服務器、密碼。
  let 替换字串; // 替換字串。
  if (是恢复) { // 如果是恢復。
    替换字串 = "ssr://" + 路引安全六四编码(链接.replace(密码, 路引安全六四编码(替换清单[路引安全六四解码(密码)])).replace(服务器, 替换清单[服务器])); // 替換字串。
  } else {
    const 随机密码 = 生成随机串(12); // 隨機密碼。
    const 随机域名 = 生成随机串(12) + ".com"; // 隨機域名。
    替换清单[随机域名] = 服务器; // 替換清單。
    替换清单[随机密码] = 路引安全六四解码(密码); // 替換清單。
    替换字串 = "ssr://" + 路引安全六四编码(链接.replace(服务器, 随机域名).replace(密码, 路引安全六四编码(随机密码))); // 替換字串。
  }
  return 替换字串; // 回替換字串。
}

function 替换暗号变种(链接, 替换清单, 是恢复) { // 替換暗號變種。
  const 随机识别码 = 生成随机识别码(); // 隨機識別碼。
  const 随机域名 = 生成随机串(10) + ".com"; // 隨機域名。
  const 正则匹配火箭样式 = 链接.match(/vmess:\/\/([A-Za-z0-9-_]+)\?(.*)/); // 正規匹配火箭樣式。
  if (正则匹配火箭样式) { // 如果匹配到。
    const 六四数据 = 正则匹配火箭样式[1]; // 六四數據。
    const 正则匹配 = 路引安全六四解码(六四数据).match(/(.*?):(.*?)@(.*):(.*)/); // 正規匹配。
    if (!正则匹配) // 如果沒匹配到。
      return; // 回空。
    const [, 加密, 识别码, 服务器, 端口] = 正则匹配; // 加密、識別碼、服務器、端口。
    替换清单[随机域名] = 服务器; // 替換清單。
    替换清单[随机识别码] = 识别码; // 替換清單。
    const 新字串 = 路引安全六四编码(`${加密}:${随机识别码}@${随机域名}:${端口}`); // 新字串。
    const 结果 = 链接.replace(六四数据, 新字串); // 結果。
    return 结果; // 回結果。
  }
  const 正则匹配狐狸样式 = 链接.match(/vmess1:\/\/(.*?)@(.*):(.*?)\?(.*)/); // 正規匹配狐狸樣式。
  if (正则匹配狐狸样式) { // 如果匹配到。
    const [, 识别码, 服务器] = 正则匹配狐狸样式; // 識別碼、服務器。
    替换清单[随机域名] = 服务器; // 替換清單。
    替换清单[随机识别码] = 识别码; // 替換清單。
    const 正则 = new RegExp(`${识别码}|${服务器}`, "g"); // 正規表達式。
    const 结果 = 链接.replace(正则, (match) => 字符替换(match, 识别码, 随机识别码, 服务器, 随机域名)); // 結果。
    return 结果; // 回結果。
  }
  let 临时链接 = 链接.replace(/vmess:\/\/|vmess1:\/\//g, ""); // 臨時鏈接。
  try {
    临时链接 = 路引安全六四解码(临时链接); // 解碼。
    const 正则匹配泉样式 = 临时链接.match(/(.*?) = (.*)/); // 正規匹配泉樣式。
    if (正则匹配泉样式) { // 如果匹配到。
      const 配置列表 = 正则匹配泉样式[2].split(","); // 配置列表。
      if (配置列表.length < 6) // 如果配置列表長度小於六。
        return; // 回空。
      const 服务器2 = 配置列表[1].trim(); // 服務器。
      const 识别码2 = 配置列表[4].trim().replace(/^"|"$/g, ""); // 識別碼。
      替换清单[随机域名] = 服务器2; // 替換清單。
      替换清单[随机识别码] = 识别码2; // 替換清單。
      const 正则2 = new RegExp(`${识别码2}|${服务器2}`, "g"); // 正規表達式。
      const 结果2 = 临时链接.replace(正则2, (match) => 字符替换(match, 识别码2, 随机识别码, 服务器2, 随机域名)); // 結果。
      return "vmess://" + btoa(结果2); // 回結果。
    }
    const 秘档数据 = JSON.parse(临时链接); // 秘檔數據。
    const 服务器 = 秘档数据.add; // 服務器。
    const 识别码 = 秘档数据.id; // 識別碼。
    const 正则 = new RegExp(`${识别码}|${服务器}`, "g"); // 正規表達式。
    let 结果; // 結果。
    if (是恢复) { // 如果是恢復。
      结果 = 临时链接.replace(正则, (match) => 字符替换(match, 识别码, 替换清单[识别码], 服务器, 替换清单[服务器])); // 結果。
    } else {
      替换清单[随机域名] = 服务器; // 替換清單。
      替换清单[随机识别码] = 识别码; // 替換清單。
      结果 = 临时链接.replace(正则, (match) => 字符替换(match, 识别码, 随机识别码, 服务器, 随机域名)); // 結果。
    }
    return "vmess://" + btoa(结果); // 回結果。
  } catch (错处) { // 捕獲錯誤。
    return; // 回空。
  }
}

function 替换暗号(链接, 替换清单, 是恢复) { // 替換暗號。
  const 随机密码 = 生成随机串(12); // 隨機密碼。
  const 随机域名 = 随机密码 + ".com"; // 隨機域名。
  let 替换字串; // 替換字串。
  let 临时链接 = 链接.slice("ss://".length).split("#")[0]; // 臨時鏈接。
  if (临时链接.includes("@")) { // 如果包含@。
    const 正则匹配1 = 临时链接.match(/(\S+?)@(\S+):/); // 正規匹配。
    if (!正则匹配1) { // 如果沒匹配到。
      return; // 回空。
    }
    const [, 六四数据, 服务器] = 正则匹配1; // 六四數據、服務器。
    const 正则匹配2 = 路引安全六四解码(六四数据).match(/(\S+?):(\S+)/); // 正規匹配。
    if (!正则匹配2) { // 如果沒匹配到。
      return; // 回空。
    }
    const [, 加密, 密码] = 正则匹配2; // 加密、密碼。
    if (是恢复) { // 如果是恢復。
      const 新字串 = 路引安全六四编码(加密 + ":" + 替换清单[密码]); // 新字串。
      替换字串 = 链接.replace(六四数据, 新字串).replace(服务器, 替换清单[服务器]); // 替換字串。
    } else {
      替换清单[随机域名] = 服务器; // 替換清單。
      替换清单[随机密码] = 密码; // 替換清單。
      const 新字串 = 路引安全六四编码(加密 + ":" + 随机密码); // 新字串。
      替换字串 = 链接.replace(六四数据, 新字串).replace(/@.*:/, `@${随机域名}:`); // 替換字串。
    }
  } else {
    try {
      const 解码值 = 路引安全六四解码(临时链接); // 解碼值。
      const 正则匹配 = 解码值.match(/(\S+?):(\S+)@(\S+):/); // 正規匹配。
      if (!正则匹配) { // 如果沒匹配到。
        return; // 回空。
      }
      const [, , 密码, 服务器] = 正则匹配; // 密碼、服務器。
      替换清单[随机域名] = 服务器; // 替換清單。
      替换清单[随机密码] = 密码; // 替換清單。
      替换字串 = "ss://" + 路引安全六四编码(解码值.replace(/:.*@/, `:${随机密码}@`).replace(/@.*:/, `@${随机域名}:`)); // 替換字串。
      const 散列部分 = 链接.match(/#.*/); // 散列部分。
      if (散列部分) // 如果有散列部分。
        替换字串 += 散列部分[0]; // 加上散列部分。
    } catch (错处) { // 捕獲錯誤。
      return; // 回空。
...
function 替换暗门(链接, 替换清单, 是恢复) { // 替換暗門。
  const 随机识别码 = 生成随机识别码(); // 隨機識別碼。
  const 随机域名 = 生成随机串(10) + ".com"; // 隨機域名。
  const 正则匹配 = 链接.match(/(vless|trojan):\/\/(.*?)@(.*):/); // 正規匹配。
  if (!正则匹配) { // 如果沒匹配到。
    return; // 回空。
  }
  const [, , 识别码, 服务器] = 正则匹配; // 識別碼、服務器。
  替换清单[随机域名] = 服务器; // 替換清單。
  替换清单[随机识别码] = 识别码; // 替換清單。
  const 正则 = new RegExp(`${识别码}|${服务器}`, "g"); // 正規表達式。
  if (是恢复) { // 如果是恢復。
    return 链接.replace(正则, (match) => 字符替换(match, 识别码, 替换清单[识别码], 服务器, 替换清单[服务器])); // 回替換。
  } else {
    return 链接.replace(正则, (match) => 字符替换(match, 识别码, 随机识别码, 服务器, 随机域名)); // 回替換。
  }
}

function 替换狂暴(链接, 替换清单) { // 替換狂暴。
  const 正则匹配 = 链接.match(/hysteria:\/\/(.*):(.*?)\?/); // 正規匹配。
  if (!正则匹配) { // 如果沒匹配到。
    return; // 回空。
  }
  const 服务器 = 正则匹配[1]; // 服務器。
  const 随机域名 = 生成随机串(12) + ".com"; // 隨機域名。
  替换清单[随机域名] = 服务器; // 替換清單。
  return 链接.replace(服务器, 随机域名); // 回替換。
}

function 替换歪门(歪门物件, 替换清单) { // 替換歪門。
  if (!歪门物件.proxies) { // 如果沒有代理。
    return; // 回空。
  }
  歪门物件.proxies.forEach((代理) => { // 遍歷代理。
    const 随机密码 = 生成随机串(12); // 隨機密碼。
    const 随机域名 = 随机密码 + ".com"; // 隨機域名。
    const 原版服务器 = 代理.server; // 原版服務器。
    代理.server = 随机域名; // 代理服務器。
    替换清单[随机域名] = 原版服务器; // 替換清單。
    if (代理.password) { // 如果有密碼。
      const 原版密码 = 代理.password; // 原版密碼。
      代理.password = 随机密码; // 代理密碼。
      替换清单[随机密码] = 原版密码; // 替換清單。
    }
    if (代理.uuid) { // 如果有UUID。
      const 原版识别码 = 代理.uuid; // 原版識別碼。
      const 随机识别码 = 生成随机识别码(); // 隨機識別碼。
      代理.uuid = 随机识别码; // 代理識別碼。
      替换清单[随机识别码] = 原版识别码; // 替換清單。
    }
  });
  return 歪门邪道.dump(歪门物件); // 回傾倒後的歪門物件。
}

function 路引安全六四编码(输入) { // 路引安全六四編碼。
  return btoa(输入).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, ""); // 編碼。
}

function 路引安全六四解码(输入) { // 路引安全六四解碼。
  const 补齐 = 输入 + "=".repeat((4 - 输入.length % 4) % 4); // 補齊。
  return atob(补齐.replace(/-/g, "+").replace(/_/g, "/")); // 解碼。
}

function 生成随机串(长度) { // 生成隨機串。
  return Math.random().toString(36).substring(2, 长度); // 生成隨機串。
}

function 生成随机识别码() { // 生成隨機識別碼。
  return "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g, function (c) { // 生成隨機識別碼。
    const r = Math.random() * 16 | 0; // 隨機數。
    const v = c == "x" ? r : r & 3 | 8; // 值。
    return v.toString(16); // 轉十六進制。
  });
}

function 解析数据(数据) { // 解析數據。
  try {
    return { format: "base64", data: 路引安全六四解码(数据) }; // 回base64格式和解碼數據。
  } catch (六四错处) { // 捕獲六四錯誤。
    try {
      return { format: "yaml", data: 歪门邪道.load(数据) }; // 回yaml格式和加載數據。
    } catch (歪门错处) { // 捕獲歪門錯誤。
      return { format: "unknown", data: 数据 }; // 回未知格式和數據。
    }
  }
}

function 字符替换(匹配, ...替换对) { // 字符替換。
  for (let i = 0; i < 替换对.length; i += 2) { // 循環替換對。
    if (匹配 === 替换对[i]) { // 如果匹配。
      return 替换对[i + 1]; // 回替換。
    }
  }
