...
var 要浮点 = (function() {
  ...
  function 解析山寨浮点(data) {
    if (data === null)
      return false;
    if (!山寨浮点模式.test(data) || // 小技巧：不允許整數以底線結尾
      // 可能需要更新正則表達式並檢查速度
...
var 要装货机 = (function() {
  ...
  function 组节点(state, parentIndent, nodeContext, allowToSeek, allowCompact) {
    ...
    if (indentStatus === 1 || 上下文块外 === nodeContext) {
      ...
      if (indentStatus === 1) {
        if (allowBlockCollections && (读塊序列(state, blockIndent) || 读块映射(state, blockIndent, flowIndent)) || 读流集合(state, flowIndent)) {
          hasContent = true;
        } else {
        ...
        hasContent = allowBlockCollections && 读塊序列(state, blockIndent);
      }
    }
    if (state.tag === null) {
      if (state.anchor !== null) {
